...
class YPipeWidget(QWidget):
    ...
    def paintEvent(self, event=None):
        ...
        painter.drawPolygon(QPolygon([QPoint(ax, ay), QPoint(bx, by),
                                      QPoint(cx, cy), QPoint(ix, iy)]))
        ...
        painter.setBrush(QBrush(gradient))
        painter.drawPolygon(QPolygon([QPoint(cx, cy), QPoint(dx, dy),
                                      QPoint(ex, ey), QPoint(fx, fy)]))
        ...
        painter.drawPolygon(QPolygon(
                [QPoint(cx, cy), QPoint(fx, fy), QPoint(gx, gy), QPoint(hx, hy), QPoint(ix, iy)]))

        painter.setPen(Qt.black)
        painter.drawPolyline(QPolygon([QPoint(ax, ay), QPoint(ix, iy), QPoint(hx, hy)]))
        painter.drawPolyline(QPolygon([QPoint(gx, gy), QPoint(fx, fy), QPoint(ex, ey)]))
        painter.drawPolyline(QPolygon([QPoint(bx, by), QPoint(cx, cy), QPoint(dx, dy)]))
...
if __name__ == "__main__":
    ...
    form = YPipeWidget()
    form.signal.connect(valueChanged)
